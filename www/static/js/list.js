...
List.prototype.load = function(options,filter){
	...
	$.getJSON(API_ROOT + "/dodavatele",options,function(data){
		...
		$.each(data.result.dodavatele,function(i,dodavatel){
			...
			var ico = dodavatel.ico_st ? ("00000000" + dodavatel.ico_st).slice(-8) : null;
			var dodavatel_id = dodavatel.id;
			
			dodavatel_dom.data("ico",ico);
			
			var dodavatelData = {
				"nazev": dodavatel.nazev_st,
				"ico": ico,
...
List.prototype.loadFirm = function(dodavatel_id,callback){
	...
	$.getJSON(API_ROOT + "/dodavatel",{"dodavatel":dodavatel_id},function(data){
		
		var target = list.dom.find(".detail .dodavatel");
		
		var kamos = data.result.kamos;
		var db = data.result.db;
		
		var ico = db.ico_st ? ("00000000" + db.ico_st).slice(-8) : null;
		
		var dodavatel_data = {
			"nazev": kamos && kamos.company_name ? kamos.company_name : db.nazev_st,
			"ico": ico,
